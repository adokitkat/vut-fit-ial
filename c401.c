...
**/
	...
	tBSTNodePtr tmpNodePtr;

	if (!(*RootPtr)) return;	// Uzel s klicem ve stromu neni, nedelam nic
	...
	if (K < (*RootPtr)->Key) 
		...
		if ((*RootPtr)->LPtr) {
			tmpNodePtr = (*RootPtr)->LPtr;
		
		} else if ((*RootPtr)->RPtr) {
			tmpNodePtr = (*RootPtr)->RPtr;
