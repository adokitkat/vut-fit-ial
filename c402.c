...
**/
	...
	Leftmost_Preorder(RootPtr, &p);

	while (!SEmptyP(&p))
		Leftmost_Preorder(STopPopP(&p)->RPtr, &p);
}
...
**/
	...
	Leftmost_Inorder(RootPtr, &p);

	while (!SEmptyP(&p)) {
		RootPtr = STopPopP(&p);
		Leftmost_Inorder(RootPtr->RPtr, &p);
		BTWorkOut(RootPtr);
	}
}
...
**/
	...
	Leftmost_Postorder(RootPtr, &p, &b);
	while(!SEmptyP(&p)){
		if((RootPtr = STopPopP(&p)) && STopPopB(&b)) {
			SPushB(&b, false);
			SPushP(&p, RootPtr);
			Leftmost_Postorder(RootPtr->RPtr, &p, &b);
		} else
			BTWorkOut(RootPtr);
	}
}
